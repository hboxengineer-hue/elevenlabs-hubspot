...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── Unified Email & Name Matching (Prioritizes Sheet Data) ──
    const customVars = metadata?.custom_variables || webhookData?.conversation_initiation_client_data?.dynamic_variables || {};
    const contactEmail = customVars.Email || customVars.email || dataCollection?.contact_email?.value || '';
    const contactName  = customVars.Name  || customVars.name  || dataCollection?.contact_name?.value  || '';

    // ── Debug Logs for Vercel ──
    console.log(`📞 Webhook received for Phone: ${callerPhone}`);
    console.log(`📦 Dynamic/Custom variables found:`, JSON.stringify(customVars, null, 2));
    console.log(`📧 Extracted Email: "${contactEmail}" | Name: "${contactName}"`);
